...
class LoginWindow(QWidget):
    def __init__(self):
        ...
        self.setFixedSize(500,600)#--csixa
        self.setStyleSheet("background-color:#f0f0f0")
        layout = QVBoxLayout()

        logo_label = QLabel()
        pixmap = QPixmap("logo.png")  
        self.setWindowIcon(QIcon("este.png"))
        ...
        login_button.clicked.connect(self.handle_Login)  # Método de login a ser implementado
  ...
  def conectar(self):
        try:
            ...
            if conn.is_connected():
                print("Conexão com o banco de dados foi bem-sucedida!")  # Log de sucesso
                return conn
            else:
                QMessageBox.critical(self, "Erro de Conexão", "Falha ao conectar ao banco de dados.")
                return None
        except mysql.connector.Error as err:
            QMessageBox.critical(self, "Erro de Conexão", f"Erro ao conectar: {err}")
            print(f"Erro de conexão: {err}")  # Log detalhado do erro
            return None

        def adicionar_usuario(login,senha):
             conexao = conectar()
             sql = "INSERT INTO login (email, senha_hash) VALUES (%s, )"
             valores = login,senha
             cursor.exexute(sql,valores)
             conexao.commit()
             cursor.close()
             conexao.close()
